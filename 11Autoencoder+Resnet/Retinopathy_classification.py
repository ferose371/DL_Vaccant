...
if __name__ == "__main__":
    ...
    AutoencoderModel = am.ResidualAutoencoder()  # Instantiate your autoencoder model class
    Model1 = AutoencoderModel.build_model()
    print("train generator", train_generator)
    autoencoder_history = Model1.fit(train_generator, epochs=2, validation_data=validate_generator)
    autoencoder_test_loss = Model1.evaluate(test_generator)
    print(f'Test Loss:{autoencoder_test_loss}')
    Model1.save("my_autoencoder+Resnet_model.h5")

    model_save_path = "autoencoder+Resnet_model.h5"
    ...
    print("The Autoencoder+Resnet architecture is")
